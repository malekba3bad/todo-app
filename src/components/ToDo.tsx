    ...
    <div className="bg-white py-3 h-screen">
      ...
      <div className="flex justify-center items-center flex-col">
       ...
       <div className="max-w-2xl mx-auto mt-5 flex flex-col items-center justify-center gap-4">
        {tasks.length > 0? tasks.filter((task) => task !== "").reverse().map((task, i) => (
        <div key={i} className="w-full border border-indigo-700 hover:bg-indigo-600 hover:text-white cursor-pointer duration-200 py-5 px-10 bg-gray-100  text-black font-extrabold rounded-xl duration-150">
          {task}
</div> 
)) : <p className="text-2xl text-center capitalize text-indigo-700 font-bold">You have no added tasks! ☹️ <br /> Add a task to get started .... </p>
}
